...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    console.log("NodeId: ", nodeId, Number(nodeId));
    if (!user) {
      alert("유저없음");
      return;
    }
    const { data, error } = await supabase.from("subnode").insert({
      title: title,
      content: content,
      writer_id: user?.id,
    ...
    <div className="ml-20">
      <form onSubmit={handleSubmit}>
        <div className="w-11/12 flex justify-between mt-20">
          ...
          <Button type="submit" className="px-7 py-5 text-[#4c84ff]">
            완료
          </Button>
        </div>
        <div className="border border-[#848484] rounded-3xl min-h-[50rem] w-11/12 bg-[#1e1e1e] p-16 mr-20 mt-14">
          <Input
            ...
            className="text-[#4c84ff] font-bold text-6xl w-full py-4"
            value={title}
